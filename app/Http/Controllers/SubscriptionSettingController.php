...
class SubscriptionSettingController extends Controller
{
    ...
    public function subscriptionStore(Request $request)
    {
        ...
        return redirect("app/subscription/list")->withErrors([
            "Success",
    ...
    public function uploadFile(Request $request)
    {
        ...
        foreach ($rowData as $key => $singleR) {
            if (
                $singleR["sno_number"] != "" &&
                $singleR["sno_number"] != "MyAppID No."
            ) {
                // echo '<pre>';print_r($singleR['type']);
                $subscription = new Transaction();

                // $subscription->invoice = $singleR['invoice'];
                $checkTransaction = Transaction::orwhere(
                    "sno_number",
                    $singleR["sno_number"]
                )
                    ->where(
                        "expire",
                        date("Y-m-d", strtotime($singleR["expire"]))
                    )
                    ->first();
                if ($checkTransaction) {
                } else {
                    $SubscriptionSetting = SubscriptionSetting::where(
                        "description",
                        $singleR["type"]
                    )->first();

                    echo $kk . "--" . $singleR["sno_number"];
                    echo "<br>";

                    $checkSubscription = CustomerSubscription::where(
                        "sno_number",
                        $singleR["sno_number"]
                    )->first();

                    if ($checkSubscription) {
                        $check2 = CustomerSubscription::where(
                            "sno_number",
                            $singleR["sno_number"]
                        )->get();

                        foreach ($check2 as $key => $value1) {
                            $checkSubscription1 = CustomerSubscription::where(
                                "id",
                                $value1->id
                            )->first();
                            //   echo $singleR['expire'];
                            // echo '<pre>';print_r($checkSubscription);die;
                            $checkSubscription1->expire = date(
                                "Y-m-d",
                                strtotime($singleR["expire"])
                            );
                    ...
                    $subscription->start = date(
                        "Y-m-d",
                        strtotime("- 1 year", strtotime($singleR["expire"]))
                    );
    ...
    public function customerUpdate($id, Request $request)
    {
        ...
        $checkSubs = CustomerSubscription::where(
            "customer_id",
            $request->Organization_Number
        )->get();
        ...
        foreach ($all["code"] as $keyA => $singleA) {
            ...
            $subscription->start = @date(
                "Y-m-d",
                strtotime($all["start"][$keyA])
            );
            $subscription->expire = @date(
                "Y-m-d",
                strtotime($all["expire"][$keyA])
            );
    ...
    public function customerSubscription(Request $request)
    {
        ...
        if ($columnName == "total_price") {
            $records = $records->orderBy(
                \DB::raw("CAST(total_price AS SIGNED INTEGER)"),
                $columnSortOrder
            );
        } else {
        ...
        foreach ($records as $record) {
            ...
            // Get customer name
            $custNo = CustomerSubscription::where(
                "sno_number",
                $record["sno_number"]
            )->first();
            $custName = Cust::where(
    ...
    {
        $this->data["edit"] = Transaction::where("id", $id)
            ->first()
            ->toArray();
        //dd($this->data['edit']['customer_id']);
        $this->data["Organization_Name"] = Cust::where(
            "Organization_Number",
            $this->data["edit"]["customer_id"]
        )->first();
        $this->data["subscriptions"] = SubscriptionSetting::get();
    ...
    public function customerSubscriptionList(Request $request)
    {
        ...
        foreach ($prod as $key => $value) {
            $statusArray = [0, 2];
            $prod = SubscriptionSetting::where("cat", $value["id"])
                ->get()
                ->pluck("code")
                ->toArray();
            $sum = Transaction::whereDate("expire", ">=", date("Y-m-d"))
        ...
        $invDates = Transaction::orderBy("expire")
            ->get()
            ->pluck("expire")
            ->toArray();
        ...
        // valid contract
        $statusArray = [0, 2];
        $allrecord = Transaction::whereDate("expire", ">=", date("Y-m-d"))
            ->whereIn("status", $statusArray)
            ->get();
        ...
        $renewAll = Transaction::where("status", 0)
            ->whereMonth("expire", date("m"))
        ...
        $lastYear = $Lastyear;
        $renewAllPer = Transaction::whereMonth("expire", $lastMonth)
            ->whereYear("expire", $Lastyear)
            ->get();
        $renewCountPer = 0;
        ...
        for ($i = 1; $i <= 12; $i++) {
            $monthSum = Transaction::whereYear("expire", $year)->whereMonth(
                "expire",
                $i
            );
            if ($type11) {
            ...
            // Last Year
            $lastMonthSum = Transaction::whereYear(
                "expire",
                $Lastyear
            )->whereMonth("expire", $i);
            $last3yr = Transaction::whereYear("expire", $Last3year)->whereMonth(
                "expire",
                $i
            );
            if ($type11) {
            ...
            $lst3Year[] = $last3yr->sum("total_price");
        }
        //echo '<pre>';print_r($lst3Year);die;
