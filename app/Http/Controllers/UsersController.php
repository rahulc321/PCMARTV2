...
{ 
  ...
  //echo count($less30MinutesChartLogin);die;
   ...
   // echo '<pre>';print_r($rating);die;
    ...
    $records =\DB::table('ticket')
       ...
       ->select('ticket.*','ticket.id as tid','ticket_assign.*','ictran.Organization_Name as oname','users.name as user','ticket.created_at as cdate','ticket.status as tstatus','feedback.*','a1.name as asignName')
   ...
   // echo '<pre>';print_r($records);die;
    ...
    $records = Ictran::with('getCountInvoice')->where(function ($query) use ($searchValue) {
    ...
    $totalRecords =$totalRecordswithFilter;
     ...
     foreach($records as $record){
        ...
        $data_arr[] = array(
          ...
          "due_date" => ($due==0) ? date('Y',strtotime($record['Due_date'])) : date('d-m-Y',strtotime($record['Due_date']))
        );
     }
  ...
  public function settingsUpdate(Request $request,$id){
   ...
   ->take($rowperpage)
    ...
    public function deleteInfo($id){
        ...
        return redirect('app/info')->withErrors(['Success', 'You have successfully Deleted !!!']);
    }
    ...
    public function addInfo(Request $request){
        ...
        $info->save();
        return redirect('app/info')->withErrors(['Success', 'You have successfully Added !!!']);
  ...
  public function trainingUpload(Request $request){
   ...
   //  echo '<pre>';print_r($actual_data);die;
            ...
            foreach ($actual_data as $key => $value) {
                ...
                $checkCustNo= Training::where('Invoice',$value['refno'])->where('code',$value['itemno'])->first();
               ...
               // / if($value['refno']=='TI107806'){
                ...
                $custName= Cust::where('Organization_Number',$value['custno'])->first();
                $trainingSetting= TrainingSetting::where('code',$value['itemno'])->first();
                echo $value['refno'].'--'.@$custName->Organization_Name; echo '<br>';
                $values = new Training;
                if($value['refno'] !=""){
                ...
                $sett = TrainingSetting::where('code',$value['itemno'])->first();
  ...
  public function subscriptionictran(Request $request){
        ...
        // echo '<pre>';print_r($actual_data);die;
            ...
            foreach ($actual_data as $key => $value) {
               // $checkCustNo = Transaction::orwhere('sno_number',$value['brem2'])->first();
                ...
                $checkCustNo= Transaction::where('invoice',$value['refno'])->where('sno_number',$value['brem2'])->first();
                ...
                $action=0;
                if($request->start !="" && $request->to !="" && $request->start <= $value['itemno'] && $request->to >= $value['itemno'] && $value['type'] != 'RC'){
              ...
              //  echo $value['brem2'];
                ...
                $new=CustomerSubscription::where('code',$value['itemno'])->where('sno_number',$value['brem2'])->first();
              ...
              // echo '<pre>';print_r($new);die;
               ...
               // echo '<pre>';print_r($new->id);
                $custName= Cust::where('Organization_Number',$value['custno'])->first();
                $new1=CustomerSubscription::where('id',$new->id)->first();
                //$new1->start = date('Y-m-d',strtotime($value['date']));
                $new1->expire = date('Y-m-d', strtotime('+ 1 year', strtotime($new->expire)));
                $new1->save();
                if($value['type'] != 'RC'){
                  $success[]=$value['refno'].' - '.$value['brem2'];
                }
                //echo $value['brem2']; echo '----------------'; echo '<br>';
                 ...
                 $values->start = date('Y-m-d',strtotime($value['date']));
                 $values->expire = date('Y-m-d', strtotime('+ 1 year', strtotime($new->expire)));

                 if($new->user > 1){
                    $values->total_price =$value['amt']*$new->user;
                 }else{
    ...
    public function copyCompanyname(){
      ...
      foreach ($transaction as $key => $value) {
        $transactionUpdate = Transaction::where('id',$value->id)->first();
         $custName= Cust::where('Organization_Number',@$value->customer_id)->first();
         if($custName){
    ...
    public function serviceContract (Request $request){
        ...
        $prod= ProductCat::where('type',1)->get()->toArray();

          //echo '<pre>';print_r($prod);die;
          $arrayChart=[];
          foreach ($prod as $key => $value) {
          $statusArray= [0,2];
          $prod= Product::where('cat',$value['id'])->get()->toArray();
          ...
          $sum= Ictran::whereDate('Due_date','>=',date('Y-m-d'))->whereIn('renew_status',$statusArray)->whereIn('Support_Type',array_unique($cC))->sum('Price_RM');
        ...
        $invDates= Ictran::orderBy('invoice_date')->get()->pluck('invoice_date')->toArray();
  ...
  for ($i=1; $i <=12 ; $i++) { 
   ...
   ->take($rowperpage);
    if($columnName=='Price_RM'){
      $records=$records->orderBy(\DB::raw('CAST(Price_RM AS SIGNED INTEGER)'),$columnSortOrder);
    }else{
   ...
   public function ticketStore($id, Request $request){
       ...
       ->select('ticket.*','ticket.id as tid','ticket_assign.*','ictran.Organization_Name as oname','users.name as user','ticket.created_at as cdate','ticket.status as tstatus')->where('ticket.id',$ticket->id)->first();
   ...
   public function getCount (){
    ...
    // }
     ...
     // $notCount= Notification::where('user_id',\Auth::Id())->where('status',0)->count();
      $Notification= Notification::join('ticket', 'ticket.id', '=', 'notification.ticket_id');
     // if(\Auth::user()->user_type != 1){
      $Notification= $Notification->where('notification.user_id',\Auth::Id());
     //}


      $notCount= $Notification->where('ticket.status',0)->where('notification.status',0)->select('notification.*','ticket.status as tstatus','ticket.id as tid')->orderBy('ticket.id','desc')->count();
      echo $notCount;
   }
   ...
   public function getNotification (){
    ...
    $Notification= $Notification->where('ticket.status',0)->select('notification.*','ticket.status as tstatus','ticket.id as tid')->orderBy('ticket.id','desc')->get();
   // echo '<pre>';print_r(count($Notification));die;
    if(count($Notification) > 0){
    foreach ($Notification as $key => $value) {

      $assignTicket= AssignTicket::where('ticket_id',$value->tid)->first();
    ...
    ?>
      ...
      color: green;
        ...
        <li class="scrollable-container media-list" style="background-color:<?=@$bg?> ">
          <a class="<?php if(\Auth::user()->user_type != 1 || \Auth::Id()== $value->user_id){ echo 'seen'; } ?> d-flex justify-content-between" href="javascript:;" data="<?=$value->id?>">
              <div class="media d-flex align-items-center">
                <div class="media-left pr-0">
                  <div class="avatar mr-1 m-0"><img src="<?php  echo asset('profile').'/'.$user->profile_pic?>" alt="avatar" height="39" width="39"></div>
                </div>
                <div class="media-body">
                  ...
                  <span><small class="notification-text"><?=date('d-m-Y',strtotime($value->cdate))?></small></span>
...
<?php  }
   ...
   public function ticket (Request $request){
        ...
        $allUser= User::whereIn('id',$userIdArray)->get()->toArray();
  ...
  //echo count($less30MinutesChartLogin);die;
   ...
   // echo '<pre>';print_r($rating);die;
      ...
      $this->data['greater30MinutesG']= $greater30MinutesG;
        ...
        /****************************Start Bar Chart**********************************/
       $invDates= Ticket::whereNotNull('close_date')->orderBy('close_date')->get()->pluck('close_date')->toArray();
        //dd(array_unique($invDate));
        $dateInv=[];
        foreach ($invDates as $key => $invDate) {
           $dateInv[]= date('Y',strtotime($invDate));
       ...
       if($request->month){
        ...
        ->join('users', 'users.id', '=', 'ticket_assign.user_id');
         ...
         $monthSum= $monthSum->where('ticket.status',2)->whereYear('ticket.close_date',$year)->whereMonth('ticket.close_date',$i);
        ...
        ->select('ticket.*','ticket_assign.user_id as uuid');
         ...
         if($type11){
          $lastMonthSum= $lastMonthSum->where('ticket_assign.user_id',$type11);
         }

         $lastMonthSum= $lastMonthSum->where('ticket.status',2)->whereYear('ticket.close_date',$Lastyear)->whereMonth('ticket.close_date',$i);
    ...
    $records =\DB::table('ticket')
       ...
       ->select('ticket.*','ticket.id as tid','ticket_assign.*','ictran.Organization_Name as oname','users.name as user','ticket.created_at as cdate','ticket.status as tstatus','feedback.*','ticket.id as tid','a1.name as asignName')
        ->where(function ($query) use ($searchValue) {
        $query->orwhere('ticket_assign.phone', 'like', '%' .$searchValue . '%')
        ->orwhere('ticket.ictran_id', 'like', '%' .$searchValue . '%')
        ->orwhere('ticket_assign.contact_person', 'like', '%' .$searchValue . '%')
       ...
       if($_GET['startDate'] != "" && $_GET['endDate']){
        ...
        $endDate= date('Y-m-d',strtotime($_GET['endDate']));
        if($status==2){
        $records= $records->whereBetween('ticket.close_date', [$startDate.' 00:00:00', $endDate.' 23:59:59']);
        }else{
          $records= $records->whereBetween('ticket.created_at', [$startDate.' 00:00:00', $endDate.' 23:59:59']);
        }
       }
...
// For close ticket
 ...
 public function ticketClose($id){
    ...
    if($_GET['month'] != "" && $_GET['year']){
        ...
        $records= $records->whereYear('expire', $_GET['year'])
   ...
   // echo '<pre>';print_r($records);die;
    ...
    $records = Ictran::leftjoin('arcust', 'arcust.Organization_Number', '=', 'ictran.CUSTNO')
    ...
    ->where(function ($query) use ($searchValue) {
       ...
       ->orwhere('ictran.Support_Type', 'like',$searchValue . '%')
       ->orwhere('ictran.Price_RM', 'like', '%' .$searchValue . '%')
       ->orwhere('arcust.Primary_Email', 'like', '%' .$searchValue . '%');
        });
...
public function sendEmail(Request $request){
       ...
       //echo '<pre>'; print_r($this->data['info']);die;
        ...
        $this->data['info']= Info::where('id',$prd->company_name)->first();
        ...
        //return view('emails.marketing',$this->data);die;
          ...
          $records= $records->whereYear('expire', $request->year)
          ->whereMonth('expire', $request->month);
          // ->where('customer_id','30AE/004');
          /* $this->data['m'] = $request->month;
          $this->data['y']= $request->year;*/
          $records=$records/*->groupBy('CUSTNO')*/->where('status',0)->get()->toArray();
         // dd($records);
        $actArray=[];
          foreach ($records as $key => $value) {
   ...
   // return view('emails.marketing',$this->data);die;
    ...
    public function categoryStore(Request $request){
        ...
        return redirect('app/category')->withErrors(['Success', 'You have successfully added !!!']);
    }
  ...
  public function categoryUpdate(Request $request,$id){
        ...
        return redirect('app/category')->withErrors(['Success', 'You have successfully updated !!!']);
    }
    ...
    public function categoryDelete($id){
        ...
        if($Product){ 
            return redirect('app/category')->withErrors(['Success', 'You have successfully deleted !!!']);
        }
    }
    ...
    public function trainingSettingStore(Request $request){
        ...
        return redirect('app/trainingSetting')->withErrors(['Success', 'You have successfully added !!!']);
    }
  ...
  public function trainingSettingUpdate(Request $request,$id){
        ...
        return redirect('app/trainingSetting')->withErrors(['Success', 'You have successfully updated !!!']);
    }
    ...
    public function trainingSettingDelete($id){
        ...
        if($Product){ 
            return redirect('app/trainingSetting')->withErrors(['Success', 'You have successfully deleted !!!']);
        }
    }
    ...
    public function training(Request $request){
      ...
      $trainingId= Schedulesession::get()->pluck('trainingId')->toArray();
      ...
      $this->data['thisMonthValue']= Training::whereMonth('invoice_date',date('m'))->whereYear('invoice_date',date('Y'))->sum('value');
       ...
       $session2 = Training::where('session1','c')->where('noOfSession',2)->get();
       $k=0;
       foreach ($session2 as $key => $value) {
          if(count($value['getCountDta'])== '1_2' || $value['getCountDta']=='none'){
        $k++;
      ...
      // echo '<pre>';print_r(count($aa));die;
       ...
       if($request->startDate != "" && $request->endDate){
        ...
        $allDta= $allDta->whereBetween('invoice_date', [$startDate.' 00:00:00', $endDate.' 23:59:59']);
       }

       if($request->product != ""){

         
        $allDta= $allDta->orwhere('product', 'like', '%' .$request->product . '%');
       }

       if($request->trainer != ""){

        $trainingId= Schedulesession::where('trainerId',$request->trainer)->get()->pluck('trainingId')->toArray();
        $allDta= $allDta->whereIn('id',$trainingId);
       }
       if($request->status != ""){

        $trainingId= Schedulesession::where('status',$request->status)->get()->pluck('trainingId')->toArray();
        $allDta= $allDta->whereIn('id',$trainingId);
       }
       ...
       if($request->session != "" && $request->session != "none"){
         ...
         if($request->session =='1_2'){
          $allDta= $allDta->where('session1','c')->where('session2','')->where('noOfSession',2);
         }else{
       ...
       $prod= TrainingSetting::get()->toArray();
          ...
          if($request->form3Year){
            $sum= Training::whereYear('invoice_date',$request->form3Year)->where('product',$value['description'])->sum('value');
          }else{
            $sum= Training::/*whereDate('invoice_date','>=',date('Y-m-d'))*/where('product',$value['description'])->sum('value');
          }
       ...
       /****************************Start Bar Chart**********************************/
       $invDates= Training::orderBy('invoice_date')->get()->pluck('invoice_date')->toArray();
        //dd(array_unique($invDate));
        $dateInv=[];
        foreach ($invDates as $key => $invDate) {
           $dateInv[]= date('Y',strtotime($invDate));
       ...
       if($request->month){
        ...
        for ($i=1; $i <=12 ; $i++) { 
         ...
         $monthSum= Training::whereYear('invoice_date',$year)->whereMonth('invoice_date',$i);
         if($type11){
         ...
         // Last Year
         $lastMonthSum= Training::whereYear('invoice_date',$Lastyear)->whereMonth('invoice_date',$i);
          if($type11){
          $lastMonthSum= $lastMonthSum->where('product',$type11);
          }
  ...
  public function trainingUpdate(Request $request,$id){
        ...
        $setting->invoice_date= date('Y-m-d',strtotime($request->invoice_date));
        
        $setting->save();

        return redirect('app/training')->withErrors(['Success', 'You have successfully updated !!!']);
    }
    ...
    public function trainingDelete($id){
        ...
        if($Product){ 
            return redirect('app/training')->withErrors(['Success', 'You have successfully deleted !!!']);
        }
...
// For Trainer
    ...
    public function trainerStore(Request $request){
        ...
        return redirect('app/trainer')->withErrors(['Success', 'You have successfully added !!!']);
    }
  ...
  public function trainerUpdate(Request $request,$id){
        ...
        return redirect('app/trainer')->withErrors(['Success', 'You have successfully updated !!!']);
    }
    ...
    public function trainerDelete($id){
        ...
        if($Product){ 
            return redirect('app/trainer')->withErrors(['Success', 'You have successfully deleted !!!']);
        }
...
// Create session session 1 and session 2 ScheduleSession
    public function session($id,$sId){
     ...
     public function sessionView($id,$sId){
       ...
       $this->data['sId']= $sId;
       $this->data['id']= $id;
       $this->data['edit']= ScheduleSession::where('trainingId',$id)->where('sessionId',$sId)->first();
      // $this->data['sId']= $sId;
    ...
    public function ScheduleSession(Request $request,$id,$sId){
     ...
     //echo '<pre>';print_r(explode('-',$request->datetimes));die;
      ...
      return redirect('app/training')->withErrors(['Success', 'You have successfully create session !!!']); 
       // $this->data['trainers']= Trainer::get();
       // $this->data['edit']= Training::where('id',$id)->first();
       // $this->data['sId']= $sId;
    ...
    public function updatesession(Request $request,$id,$sId){
     ...
     //echo '<pre>';print_r(explode('-',$request->datetimes));die;
      ...
      $sess= ScheduleSession::where('trainingId',$id)->where('sessionId',$sId)->first();
     // $sess->trainingId= $id;
     // $sess->sessionId= $sId;
      $sess->customerId= $request->customer;
      $sess->trainerId= $request->trainerId;
    ...
    public function scheduling(Request $request){
      ...
      $this->data['schedules']= ScheduleSession::where('trainerId',$_REQUEST['trainerName'])->get();
      $this->data['schedulesList']= ScheduleSession::where('trainerId',$_REQUEST['trainerName'])->get();
    ...
    public  function schedulingDelete($id)
    {
       ...
       $ss=ScheduleSession::where('id',$id)->delete();
        return redirect('app/scheduling')->withErrors(['Success', 'You have successfully deleted !!!']); 
    }
    ...
    public  function createSchedule(Request $request)
    {
      ...
      $checkSession= ScheduleSession::where('trainerId',$request->trainerTypeName)->whereDate('date',date('Y-m-d',strtotime($request->startDate)))
         ->whereTime('startTime', '>=', date('H:i:s',strtotime($request->startTime)))  // or use $request->strtotime('time_start')
         ->whereTime('endTime', '<=', date('H:i:s',strtotime($request->endTime)))
      ...
      if($checkSession){
        ...
        \Session::flash('error', "This schedule is already booked for this person please choose another date and time.");
    ...
    // echo '<pre>';print_r($request->all());die;
      ...
      $ss->trainerId= ($request->trainerTypeName != '') ? $request->trainerTypeName : '';

      if($ccc==0){
        $custInfo = Cust::where('Organization_Number',$all['companyName'])->first();
        $ss->customerName= (@$custInfo->Organization_Name) ? @$custInfo->Organization_Name : '';
        $ss->companyName= @$custInfo->custometName;
        $ss->customerId= (@$custInfo->Organization_Number != '') ? @$custInfo->Organization_Number : '';
      ...
      return redirect('app/scheduling')->withErrors(['Success', 'You have successfully addedd !!!']);
    ...
    $scheduleSession = ScheduleSession::where('id',$all['id'])->first()->toArray();
    $trainer = Trainer::where('id',$scheduleSession['trainerId'])->first();

    //echo '<pre>';print_r($scheduleSession['checkStatus']);die;

    $custInfo = Cust::where('Organization_Number',$scheduleSession['customerId'])->first();
...
.hide{
  display: none;
    ...
    <form action="<?=url('/app/updateschedule')?>/<?=$all['id']?>" method="post" id="pf">
    ...
    <div class="modal-content">
      ...
      ?>
          ...
          <div class="form-group">
            ...
            <option value="1" <?php if($t==1){ echo 'selected'; } ?>>Training</option>
            <option value="2" <?php if($t==2){ echo 'selected'; } ?>>Onsite</option>
            <option value="3" <?php if($t==3){ echo 'selected'; } ?>>Demo</option>
            <option value="4" <?php if($t==4){ echo 'selected'; } ?>>Public Holiday</option>
            <option value="5" <?php if($t==5){ echo 'selected'; } ?>>On Leave</option>
          ...
          <div class="form-group trainingselect">
            <!-- <input type="text" name="companyName"   class="form-control" placeholder="Company Name" value="<?=$scheduleSession['customerName']?>"> -->
            
            <select class="form-control js-example-basic-single oname newClass" name="ictran_id">
                        <option value="">--Select--</option>
                        <option value="<?=$scheduleSession['customerId']?>" selected><?=@$custInfo->Organization_Name?></option>
           ...
           <input type="text" class="form-control" name="companyNamec" placeholder="Company Name13">
            ...
            <input type="text" class="form-control" name="companyName" placeholder="Company Name" value="<?=$scheduleSession['companyName']?>">
            </div>
            <!--  -->
            <?php } ?>
          ...
          <div class="form-group">
            <input type="text" name="custometName"  class="form-control cname" placeholder="Customer Name" value="<?=($scheduleSession['checkStatus']==0 ) ? @$custInfo->Attention : $scheduleSession['customerName'] ?>">
            </div>
          </div>

          <div class="col-sm-6 <?=$hide?> <?=$hide5?>" style="float: left;display: none">
          <div class="form-group">
            <input type="text" name="contactPerson"  class="form-control cname" placeholder="Contact Person" value="<?=$scheduleSession['contactPerson']?>">
            </div>
          </div>
          ...
          <div class="col-sm-6 <?=$hide?> <?=$hide5?>" style="float: left;">
          <div class="form-group">
            <input type="text" name="contactNumber"  class="form-control cnumber" placeholder="Contact Number" value="<?= ($scheduleSession['checkStatus']==0) ? @$custInfo->Primary_Phone : $scheduleSession['contactNumber']?>">
            </div>
          </div>
          ...
          <div class="col-sm-6" style="float: left;">
          <div class="form-group">
            <input type="text" name="startDate" class="form-control" id="datepicker1" placeholder="Choose Date" value="<?=date('d-m-Y',strtotime($scheduleSession['date']))?>">
            </div>
          </div>

          <div class="col-sm-6 <?=$hide?>" style="float: left;">
          <div class="form-group">
            <input type="text" name="startTime"  class="form-control" id="start1" placeholder="Start Time" value="<?=$scheduleSession['startTime']?>" />
            </div>
          </div>
          ...
          <div class="col-sm-6 <?=$hide?>" style="float: left;">
          <div class="form-group">
            <input type="text" name="endTime"  class="form-control" id="end1" placeholder="End Time" value="<?=$scheduleSession['endTime']?>" />
            </div>
          </div>

           <div class="col-sm-6 <?=$hide?> <?=$hide5?>" style="float: left;">
          <div class="form-group">
            <input type="text" name="remark" class="form-control" placeholder="Remark" value="<?=$scheduleSession['remark']?>" >
            </div>
          </div>
...
public  function updateschedule($id, Request $request){
    ...
    //echo '<pre>';print_r($request->all());die;
      ...
      if($ccc==0){
        $custInfo = Cust::where('Organization_Number',$request->ictran_id)->first();
        ///echo '<pre>';print_r($request['trainerType']);die;
        $ss->customerName= (@$custInfo->Organization_Name != '') ? $custInfo->Organization_Name : '';
        $ss->companyName= (@$custInfo->custometName !='')? $custInfo->custometName : '';
        $ss->customerId= (@$custInfo->Organization_Number !='') ? $custInfo->Organization_Number :  '';
        $ss->address= @$custInfo->Address1;
        $ss->contactPerson= @$custInfo->Attention;
        $ss->contactNumber= @$custInfo->Primary_Phone;
      ...
      return redirect('app/scheduling')->withErrors(['Success', 'You have successfully updated !!!']);
}
...
public function getTType(Request $request){
  ...
  $allData= \DB::table('trainer')->whereRaw("find_in_set($id,trainerType)")->get();
...
public function schedulingData1(Request $request){
    ...
    $records =\DB::table('schedulesession')
        ->leftjoin('trainer', 'trainer.id', '=', 'schedulesession.trainerId')
       ...
       if($_GET['startDate'] != "" && $_GET['endDate']){
        ...
        $startDate= date('Y-m-d',strtotime($_GET['startDate']));
        $endDate= date('Y-m-d',strtotime($_GET['endDate']));
        $records= $records->whereBetween('schedulesession.date', [$startDate, $endDate]);
       }
       ...
       if($request->product != ""){
        ...
        $p = $request->product;
        $records= $records->whereRaw('find_in_set("'.$p.'",`schedulesession`.`product`)');
       }
   ...
   // dd($request->all());
    ...
    return redirect('app/scheduling')->withErrors(['Success', 'You have successfully Updared !!!']);
   ...
   public function getOname(Request $request){
    ...
    $records = Ictran::where('Organization_Name', 'like', '%' . $oname . '%')->orderBy('Due_date', 'desc')->whereDate('Due_date','> ',date('Y-m-d'))->get();
...
<?php
  ...
  $array= array('is1'=>$is1,'oData'=>$oData);
